...
// ── Configuración global de axios para este módulo ──────────────────────────
//
// Alternativa profesional: configurar axios.defaults.withCredentials = true
// en un archivo separado (ej: src/services/axiosConfig.js) y importarlo
// en main.jsx para que aplique a toda la app.
//
const api = axios.create({
  baseURL: 'http://localhost:3000',
  withCredentials: true   // ✅ CORRECCIÓN #3: Envía la cookie httpOnly en cada request
});
...
const Perfil = () => {
  ...
  const cargarPerfil = () => {
    setLoading(true);
    // ✅ CORRECCIÓN #3: usamos `api` (con withCredentials) en lugar de `axios`
  ...
  const guardarPerfil = async () => {
    ...
    try {
      // ✅ CORRECCIÓN #3: withCredentials mediante la instancia `api`
      await api.put(`/api/usuarios/${user.id_usuario}`, datos);
      setPassword('');
      setMensaje({ tipo: 'ok', texto: '¡Perfil actualizado con éxito!' });
    } catch (err) {
      console.error('Error al actualizar:', err);
      // Mostramos el error real del servidor para facilitar el debugging
